...
from meeting_insights.models import ActionItem, KeyTopic
...
class TestEndToEndIntegration:
    ...
    def test_full_pipeline_with_mocks(self, sample_meeting_data, test_config):
        ...
        with patch('meeting_insights.processor.TextPostProcessor') as MockTextProcessor, \
            ...
            # 执行处理
            meeting_id = "integration_test_meeting"
            insights = processor.process(sample_meeting_data, meeting_id)
            
            # 验证结果
            assert isinstance(insights, MeetingInsights)
            assert insights.meeting_id == meeting_id
            # 修改这里：transcription_id 应该等于 meeting_id，因为 processor.py 中就是这么实现的
            assert insights.transcription_id == meeting_id
            ...
            assert insights.key_topics[0].id == "topic_1"  # 验证 id 字段
    ...
    def test_config_serialization_integration(self):
        ...
        # 从JSON重建
        config_rebuilt = NLPSettings.model_validate_json(config_json)
    ...
    def test_minimal_input_integration(self):
        ...
        minimal_transcription = TranscriptionResult(
            ...
            audio_duration=300.0,  # 原来是 duration
            language="zh"
        )
    ...
    def test_file_output_integration(self, sample_meeting_data, test_config, tmp_path):
        ...
        with patch('meeting_insights.processor.MeetingSummarizer') as MockSummarizer, \
            ...
            # 保存到临时文件 - 使用 model_dump_json 替代 json
            output_file = tmp_path / "insights_output.json"
            with open(output_file, 'w', encoding='utf-8') as f:
                # 修复：使用 model_dump_json 而不是 json()
                f.write(insights.model_dump_json(indent=2, ensure_ascii=False))
            ...
            with open(output_file, 'r', encoding='utf-8') as f:
                ...
                assert data['meeting_id'] == 'file_output_test'
