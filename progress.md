...
7. 已完成摘要 Prompt 精修与异常响应防护
- 已将摘要提示词调整为“会议纪要/决策/行动项”导向，强化结构化输出约束。
- 已新增摘要质量守卫：当模型返回无关固定话术或低质量结果时，自动降级为提取式摘要，避免污染持久化结果。
- 已补充对应单测覆盖异常模型返回场景。

8. 已修复音频转录运行时依赖报错
- 已在当前虚拟环境安装 `openai-whisper`，解决 `No module named 'whisper'` 导致的转录启动失败。
- 已增强转录路由错误处理：当 Whisper 引擎不可用时返回 503 与可执行安装建议，避免直接抛出 500 黑盒错误。

9. 已完成第 1 段真实样本端到端验收
- 已新增脚本 `scripts/run_real_audio_e2e_check.py`：自动生成会议音频（本地 TTS）、执行上传→转录→摘要全链路、输出验收报告。
- 已补齐真实运行依赖 `librosa` / `soundfile`，修复真实样本转录阶段 `No module named 'librosa'` 问题。
- 已生成验收报告 `test_reports/real_audio_e2e_report_20260315_235133.json`：链路可跑通，转录与摘要均有有效输出。
- 已修正摘要类型标记：当 LLM 结果触发质量守卫降级时，`summary_type` 现在会正确标记为 `extractive`。

10. 已修复“纪要等于转录原文 + 繁体输出 + 议题粗糙”问题
- 已增强降级摘要策略：去除 `[SPEAKER_xx]` 标记、按会议语义生成结构化纪要，不再直接拼接转录原文。
- 已引入简繁转换（OpenCC）并对摘要/执行摘要/议题名/关键词统一输出简体中文。
...
2. 再补 1 段不同口音/语速的真实样本，形成最小双样本对比验收。
...
4. 摘要质量增强
- 会议纪要导向 Prompt 精修（决策/行动项/风险）
- 异常模型返回自动降级防护
- 摘要器单测补充（无关固定话术触发降级）

### 本次提交未包含

1. 稳定可用的生产级 OpenAI 端点/网关切换（当前网关模型行为异常）
2. 更完整的端到端真实录音质量评估（含多音频样本对比）

### 下一步执行项（已进入真实样本验收阶段）

1. 确认可用的 OpenAI 端点策略：
- 方案 A：切换官方 OpenAI 端点；或
- 方案 B：提供一个稳定的兼容网关（支持正常中文摘要输出）
